...
export const CertificateVerifierPage: React.FC<CertificateVerifierPageProps> = ({
  ...
  useEffect(() => {
    ...
    if (initialCertId) {
      void handleVerify(undefined, initialCertId);
    }
  }, [initialCertId]);
  ...
  const handleVerify = async (e?: React.FormEvent, customId?: string) => {
    ...
    const targetId = (customId || certInput)
      .replace(/[^a-zA-Z0-9-]/g, '')
      .toUpperCase();
    ...
    if (data && data.isVerified) {
      try {
        ...
        // Confetti is decorative and should not affect verification.
      }
    }
  };
  ...
  return (
    <div className="relative flex min-h-dvh flex-col overflow-x-hidden bg-[#f4f7fb] text-[#111827] selection:bg-[#0066cc] selection:text-white">
      ...
      <main className="relative z-10 mx-auto flex w-full max-w-6xl flex-1 flex-col gap-8 px-4 py-8 sm:px-6 sm:py-12 lg:gap-10">
        ...
        <div className="mx-auto w-full max-w-2xl space-y-4">
          ...
          >
            ...
            <button
              ...
              className="inline-flex shrink-0 items-center justify-center gap-2 rounded-xl bg-[#0066cc] px-4 text-[10px] font-bold uppercase tracking-[0.12em] text-white shadow-lg shadow-blue-900/20 transition-all hover:bg-[#0052a3] hover:shadow-blue-900/30 disabled:cursor-not-allowed disabled:opacity-45 sm:px-7"
            >
          ...
          <div className="flex flex-wrap items-center justify-center gap-2 text-[11px] text-slate-500">
            ...
            <button
              type="button"
              onClick={() => void handleVerify(undefined, 'TTXIN26272937')}
              className="rounded-lg border border-blue-200 bg-blue-50 px-3 py-1.5 font-mono font-bold tracking-wide text-[#0066cc] transition-colors hover:border-blue-300 hover:bg-blue-100 hover:underline"
            >
              TTXIN26272937
            </button>
          </div>
      ...
      {showFooter && (
        <footer className="relative z-10 border-t border-slate-200 bg-white/95 px-4 py-6 text-center font-sans text-xs text-slate-500">
          <p>© {new Date().getFullYear()} {COMPANY_CONFIG.brandName} Academic Registry & Placement Foundry.</p>
          <p className="mt-1 text-[11px] text-slate-400">
            A Unit of{' '}
            <a href="https://xnava.in" target="_blank" rel="noreferrer" className="font-semibold text-[#0066cc] underline decoration-blue-200 underline-offset-2">
              Xnava Enterprise.
            </a>{' '}
